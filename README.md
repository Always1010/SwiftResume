...
在设置的“本地磁盘备份”中选择一个可写目录后，扩展会在每次停止编辑约 1.5 秒后更新当前简历的磁盘镜像，并至少每十分钟为发生变化的简历保留一个历史快照。首次配置会立即备份全部简历。

备份目录包含 `index.swiftresume.json`、`resumes/` 和 `history/`。浏览器重启或目录权限被系统收回后，设置页会提示重新授权；权限失效不会影响 IndexedDB 中的正常自动保存。
