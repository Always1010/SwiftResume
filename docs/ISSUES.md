# SwiftResume 问题日志

## SR-031：扩展信息依赖字段名称固定列位且无法调整顺序

- 日期：2026-09-18
- 状态：已解决
- 现象或修改背景：删除“民族”等字段后，“学历”仍固定停留在右列；自定义字段只能两列排列，而“求职状态”又按名称强制独占一行，用户无法按内容长度组织版式，也无法调整字段顺序。
- 原因分析：信息行生成逻辑通过“学历”和“求职状态”的字段名称决定列位，扩展字段数据没有保存宽度选择，编辑器也没有提供排序交互。
- 解决方案：扩展字段增加可持久化的“独占一行”设置，并提供限定在拖拽手柄上的排序；普通字段按保存顺序两两自动补位，整行字段从左列开始横跨两列，所有双列内容继续与手机、邮箱共用同一列线。读取旧数据时仅迁移原“求职状态”的整行版式，后续排版不再依赖字段名称。
- 验证方式：补充字段分组、拖拽重排、旧数据迁移、编辑器控件、HTML 预览与 Typst 导出测试，并执行完整单元测试和正式构建。
- 相关文件：`src/model/resume.ts`、`src/model/resumeLayout.ts`、`src/components/EditorPanel.tsx`、`src/components/ResumePreview.test.tsx`、`src/export/typstPdf.test.ts`、`src/storage/libraryBackup.ts`、`src/styles.css`

## SR-030：调节 HTML 排版密度时预览反复清空闪烁

- 日期：2026-09-18
- 状态：已解决
- 现象或修改背景：连续拖动紧凑／宽松滑块时，HTML 预览反复消失并显示加载提示，需要停下来等待分页，难以观察文字间距变化。
- 原因分析：每次简历变化都将页数归零并清空可见分页，重复等待字体与照片，再直接在可见容器重建页面。
- 解决方案：保留可见页面，密度 CSS 立即生效；按动画帧合并更新并取消旧任务，在隐藏容器测量后统一提交最新分页；复用页壳、未变化正文和已加载字体／照片，保持滚动位置。导出通过独立就绪状态等待最新分页，避免保留的旧页数误放行。
- 验证方式：40 个测试文件、143 项测试通过，TypeScript 检查及生产构建通过；九组真实 HTML/PDF 对照场景覆盖文字、字体样式、页数和边界，单页与多页各连续更新 90 次，检查无空白预览、同帧合并、节点复用与无重复字体加载。桌面页面连续调整密度后，页数随内容正常变化，滚动位置保持，控制台无错误；最终 PDF 使用 Poppler 渲染目检。
- 相关文件：`src/components/HtmlPrintPreview.tsx`、`src/components/ResumePreview.tsx`、`src/components/ResumeExportDialog.tsx`、`src/components/ResumeExportDialog.test.tsx`、`src/export/htmlPreviewUpdates.ts`、`src/export/htmlPreviewUpdates.test.ts`、`src/htmlPrint.css`、`scripts/html-print-fixtures.tsx`、`scripts/verify-html-print.mjs`

## SR-029：HTML 分页把可分段项目整体移走造成大块留白

- 日期：2026-09-18
- 状态：已解决
- 现象或修改背景：用户反馈 HTML 预览页底留白过多。
- 原因分析：短于一页的项目只有整体分页边界，即使当前页还有较多剩余空间，也不能容纳项目的前几个段落。
- 解决方案：同时保留条目与段落/列表项边界；优先在条目末尾分页，仅当剩余空白不超过 140 px 时坚持整体移动，否则按正文边界填充本页。
- 验证方式：七组真实浏览器 HTML/PDF 对照检查通过；新增两项目留白场景，确认第二个项目从第一页开始并跨页，逐页文字完整、无重复或越界。
- 相关文件：`src/export/htmlPagination.ts`、`scripts/html-print-fixtures.tsx`、`scripts/verify-html-print.mjs`

## SR-028：浏览器打印窗口打开后关联的编辑页无法交互

- 日期：2026-09-18
- 状态：已解决（窗口关联已隔离；原生打印对自身页面的阻塞仍由浏览器控制）
- 现象或修改背景：用户打开 HTML 打印窗口后切回编辑页，发现编辑页也无法操作。
- 原因分析：`window.print()` 会阻塞调用页面；旧入口通过 `window.open` 保留 opener，将编辑页与打印页放在关联的浏览上下文中，可能扩大阻塞范围。
- 解决方案：独立预览和打印页均通过 `noopener,noreferrer` 打开，不保留父窗口引用；继续通过 BroadcastChannel 同步预览。网页不尝试绕过浏览器原生打印或系统另存窗口自身的模态限制。
- 验证方式：窗口打开参数、路由编码回归测试；TypeScript 检查。原生系统另存窗口的模态范围取决于浏览器/操作系统，不能承诺所有窗口都可交互。
- 相关文件：`src/export/openPreviewWindow.ts`、`src/export/openPreviewWindow.test.ts`、`src/App.tsx`

## SR-027：PDF 正文与编辑区的字重、间距和富文本段落结构不一致

- 日期：2026-09-18
- 状态：已解决
- 现象或修改背景：项目等富文本模块在 PDF 中的标题层级、段距、列表缩进与 HTML 编辑区明显不同；列表中的多段文字被拼接，右对齐段落仍出现在左侧，嵌套列表符号出现缺字框。
- 原因分析：两端字体及密度参数独立维护，PDF 只加载 Regular 字体；段落被转换为强制换行，列表项跳过块级转换；默认宽度的段落块按内容收缩，默认二级列表符号不在内置字体中。
- 解决方案：编辑区与 PDF 使用同套 Noto 常规及粗体字体，PDF 字号和行高由 HTML 参数换算，模块及条目间距跟随密度；段落独立成块，保留缩进、空段、对齐、编号起点与嵌套结构；列表使用字体支持的符号。短项目条目整段分页，超过整页高度的条目允许跨页，标题与后续内容保持关联。新增桌面浏览器几何与内容回归脚本，并更新全部静态预览。
- 验证方式：37 个测试文件、135 项测试通过，TypeScript 检查及生产构建通过；`npm run test:layout` 验证短绩点、长说明、多条项目及单个超长项目，共 9 页、731 项几何与内容检查，目检所有渲染页。生成 100 套模板首页和三个场景全部页面共 105 张预览，生产包素材与源文件一致；正常桌面浏览器中确认生产版编辑区及两页 PDF 正常显示，控制台无错误。
- 相关文件：`src/export/typstPdf.ts`、`src/export/typstPdf.test.ts`、`src/export/typstInitialization.test.ts`、`src/model/resume.ts`、`src/model/resume.test.ts`、`src/styles.css`、`src/components/customEditors/ContentBodyEditor.tsx`、`public/fonts/NotoSansCJKsc-Bold.otf`、`scripts/verify-resume-layout.mjs`、`scripts/resume-layout-fixtures.tsx`、`package.json`、`README.md`、`src/templates/previewManifest.json`、`public/template-previews/`

## SR-026：教育背景固定分栏导致说明拥挤且未靠右

- 日期：2026-09-18
- 状态：已解决
- 现象或修改背景：PDF 教育背景第二行固定按 2:3 分栏，专业较短时留出大块空白，长说明局促；用户要求专业及学历位于左侧，绩点和说明同行靠右。
- 原因分析：PDF 使用固定比例且默认左对齐；HTML 的 flex 子项也会被长说明挤压。
- 解决方案：HTML 与 PDF 均让专业及学历按内容宽度占据左列（上限为可用宽度的 45%），说明占据剩余空间并靠右；长说明在右列自然换行，保留专业完整展示的空间。
- 验证方式：27 项相关回归测试通过，覆盖短绩点、空说明、显式换行及长说明；TypeScript 检查通过；实际生成经典模板与应届示例 PDF，目检应届示例确认专业与本科同行、说明右边缘与日期对齐。
- 相关文件：`src/model/resumeLayout.ts`、`src/components/ResumePreview.tsx`、`src/components/ResumePreview.test.tsx`、`src/export/typstPdf.ts`、`src/export/typstPdf.test.ts`、`src/styles.css`、`src/templates/previewManifest.json`、`public/template-previews/`

## SR-025：个人信息的联系方式与附加字段列位置不一致

- 日期：2026-09-18
- 状态：已解决
- 现象或修改背景：编辑区邮箱与学历不对齐，PDF 又将附加信息平分为三列，求职状态被推到右侧。
- 原因分析：联系方式与附加字段采用不同布局，HTML 与 PDF 分别维护字段分组及列宽。
- 解决方案：增加共用信息行模型及列宽参数；手机、邮箱和附加信息使用同一两列网格，学历固定右列，求职状态独占一行；侧栏模板保留单列以适应自身内容区域。
- 验证方式：25 项相关测试及 TypeScript 检查通过；生成经典、极简、侧栏和对半模板实际 PDF，目检经典与对半模板首页，确认邮箱和学历起点一致；更新四套预览素材。
- 相关文件：`src/model/resumeLayout.ts`、`src/model/resumeLayout.test.ts`、`src/components/ResumePreview.tsx`、`src/components/ResumePreview.test.tsx`、`src/export/typstPdf.ts`、`src/export/typstPdf.test.ts`、`src/styles.css`、`src/templates/previewManifest.json`、`public/template-previews/`

## SR-024：部分模板抬头把 Typst 方括号和照片表达式显示为正文

- 日期：2026-09-18
- 状态：已解决
- 现象或修改背景：素材目检确认极简模板的转行示例抬头出现多余的 `[` 和 `]`；检查发现外交公文、现代侧栏、对半切割及其派生模板使用相同的错误插入方式，侧栏与切割模板的照片表达式也可能直接成为正文。
- 原因分析：个人信息片段预先包装成 Typst 内容块，却又直接嵌入已有的 markup 内容块，使内层方括号成为文字；照片以代码表达式插入 markup 时遗漏 `#`，没有执行图片或背景块函数。
- 解决方案：拆分无外层括号的个人信息内容与供 `grid` 代码参数使用的内容块，四种模板的 markup 容器使用原始内容；侧栏与切割模板在照片表达式前补上 `#`。共用 `renderBase` 的派生模板自动使用修复，其他模板保持原有输出。
- 验证方式：Typst 源生成测试 17 项通过，其中覆盖四种抬头无多余内容分隔符、两种照片模板在无背景、有背景及无照片时的输出；TypeScript 类型检查通过。已重新生成 14 套受影响模板首页和转行示例全部页面，目检确认极简抬头无多余方括号，侧栏与切割模板正常显示照片。
- 相关文件：`src/export/typstPdf.ts`、`src/export/typstPdf.test.ts`
## SR-023：固定模板和场景示例在浏览时重复编译导致预览等待

- 日期：2026-09-18
- 状态：已解决
- 现象或修改背景：进入模板列表、新建简历示例与放大示例时，需要现场编译固定内容的 PDF 并绘制页面；首次浏览多张卡片产生连续等待。
- 原因分析：固定示例与用户真实内容共用实时预览组件，原有静态素材未用于展示，旧生成脚本通过固定等待时间截图也无法确认页面已完成渲染。
- 解决方案：固定模板首页和三个场景示例改为加载随扩展打包的 PNG，场景放大预览按清单纵向展示全部页面；保留用户内容试排与对比的实时 PDF。生成脚本调用同一 Typst 编译器，等待每页 PDF.js 渲染完成后输出图像，使用内容哈希命名，全部任务成功后原子替换清单，开发生成入口不进入生产包。专用生成服务关闭 HMR 与文件监听，每 20 项任务重启浏览器释放编译与渲染资源；失败报告包含任务和具体阶段。
- 验证方式：完整测试 36 文件、121 项通过，类型检查及生产构建通过；组件测试覆盖固定示例不挂载实时渲染器、对比仍使用真实内容，生成器测试覆盖错误定位和上报失败。完成 100 套模板首页及场景全部页面共 105 张图片的全量生成（约 58 秒），并在示例密度和抬头修复后更新对应素材；全部图片的哈希、尺寸及生产包副本一致，生产包无生成入口。目检三个完整示例；正常桌面浏览器中确认画廊只有图片、完整场景可纵向滚动。在与扩展相同的生产 CSP 下验证真实两页 PDF 和并排比较，控制台无错误；开发页面验证导出预览、缩放及更新后的滚动位置保持。
- 相关文件：`src/components/TemplateGallery.tsx`、`src/components/NewResumeDialog.tsx`、`src/components/TemplateThumbnailCapture.tsx`、`src/templates/staticPreviews.ts`、`src/templates/previewManifest.json`、`src/staticPreviews.css`、`src/main.tsx`、`scripts/generate-template-previews.mjs`、`public/template-previews/`

## SR-022：预览、PDF 与网页打印采用不同排版导致成品不一致

- 日期：2026-09-18
- 状态：已解决
- 现象或修改背景：工作台和模板试排中的换行、间距与最终 PDF 不同，浏览器打印还会再次以网页规则分页。
- 原因分析：预览维护 HTML/CSS 测量分页，导出使用 Typst，打印调用工作台的 `window.print()`，存在三条输出路径。
- 解决方案：全部成品预览消费 Typst PDF；以内容为键共享生成结果，合并连续编辑并拒绝过期结果；下载和打印指向同一 PDF，移除网页打印导出选项。模板缩略图同样渲染 PDF 首页并及时释放阅读器。
- 验证方式：共享文件、失败重试、连续编辑竞态和打印快捷键回归测试；在生产 CSP 下验证桌面工作台、场景缩略图、模板试排和 PDF 导出。打印链接的文件一致性以自动化测试验证，浏览器安全策略限制打开原生 PDF 打印页，未进行实体打印。
- 相关文件：`src/components/ResumePreview.tsx`、`src/components/PdfCanvasPreview.tsx`、`src/components/PdfExportDialog.tsx`、`src/components/TemplateGallery.tsx`、`src/export/pdfArtifact.ts`、`src/export/useTypstPreview.ts`、`src/export/usePdfPrintShortcut.ts`、`src/App.tsx`、`src/components/StandalonePreview.tsx`、`src/settings/appSettings.ts`、`src/typstPreview.css`

## SR-021：新增模块后未直接打开编辑器

- 日期：2026-09-18
- 状态：已解决
- 现象或修改背景：快捷添加模块后只增加了模块，用户还需要再次点击画布才能填写。
- 原因分析：新增与定位在同一轮更新中执行，定位逻辑使用旧的模块列表判断可见性，新模块尚不存在，因而没有打开编辑器。
- 解决方案：新增模块使用独立回调，一次更新模块列表与编辑目标，并在渲染后滚动定位；窄窗口同步切换到编辑视图。
- 验证方式：生产构建中新增技能模块，验证直接出现技能字段、填写骨架和富文本编辑器；模块类型字段测试及类型检查通过。
- 相关文件：`src/App.tsx`、`src/components/Sidebar.tsx`、`src/components/AddModuleDialog.tsx`、`src/components/EditorPanel.test.tsx`

## SR-020：撤销正文修改会同时丢失最近下载记录

- 日期：2026-09-18
- 状态：已解决
- 现象或修改背景：下载 PDF 后增加了一步“撤销导出记录”，继续撤销内容也会恢复旧的下载记录，导致无法稳定对照最近实际导出的内容。
- 原因分析：导出记录与可编辑内容一起进入同一份撤销快照。
- 解决方案：记录下载不新增编辑撤销步骤；撤销和重做内容时始终保留最新导出快照。
- 验证方式：回归测试验证下载前后的撤销步数、内容恢复和快照保留。
- 相关文件：`src/model/resumeHistory.ts`、`src/model/resumeHistory.test.ts`

## SR-019：多步骤操作弹窗继承固定网格导致文字重叠

- 日期：2026-09-18
- 状态：已解决
- 现象或修改背景：生产包中岗位版本弹窗的说明文字与第一组字段标签重叠，较长内容的滚动布局不自然。
- 原因分析：新的表单弹窗继承了 PDF 预览弹窗的三行网格布局，中间行被压缩，但表单实际含有多段独立内容。
- 解决方案：表单类弹窗使用普通文档流与自动高度，保留视口最大高度和内部滚动。
- 验证方式：生产包中检查岗位版本、文本导入和恢复弹窗，并复核窄窗口布局。
- 相关文件：`src/workspace.css`

## SR-018：扩展安全策略阻止 Typst 初始化

- 日期：2026-09-17
- 状态：已解决
- 现象或修改背景：生产包在 Manifest V3 同等安全策略下生成 PDF 失败，错误提示禁止动态脚本求值；普通开发页面未暴露该问题。
- 原因分析：Typst 0.7 的字体加载器与 WASM 初始化占位回调使用 `new Function`，与扩展禁止 `unsafe-eval` 的策略冲突。
- 解决方案：直接加载随扩展打包的中文字体并关闭默认字体加载；构建时将已知占位回调映射为等价静态函数，未知回调拒绝执行，保留原有安全策略。
- 验证方式：回归测试覆盖固定回调、未知代码拒绝、字体加载失败重试；生产预览服务启用与 manifest 一致的 CSP，实际生成并查看两页中文 PDF；类型检查与构建通过。
- 相关文件：`src/export/typstPdf.ts`、`src/export/typstInitialization.test.ts`、`src/export/typstCsp.ts`、`src/export/typstCsp.test.ts`、`vite.config.ts`

## SR-017：PDF 中文列表过密且教育说明挤压专业字段

- 日期：2026-09-17
- 状态：已解决
- 现象或修改背景：在真实 PDF 中检查默认示例，中文列表相邻行拥挤重叠；较长的教育说明占据大部分行宽，专业和学历被挤成窄列。
- 原因分析：Typst 列表条目固定为 1pt 间距，文字布局沿用默认大写字高和基线边界；教育说明列采用自动宽度，未限制长说明占用空间。
- 解决方案：为中文设置完整的 em 行框，列表和编号列表跟随段落行距；教育信息按 2:3 比例分配两列并增加列间距。
- 验证方式：补充导出回归测试；用默认中文示例实际生成 PDF 并检查列表、教育信息和分页；执行类型检查和正式构建。
- 相关文件：`src/export/typstPdf.ts`、`src/export/typstPdf.test.ts`

## SR-016：收起预览时浏览器打印缺少简历内容

- 日期：2026-09-17
- 状态：已解决
- 现象或修改背景：专注编辑或关闭预览后，浏览器打印缺少简历页面。
- 原因分析：页面预览被条件卸载，而打印样式又隐藏编辑画布，打印时没有剩余的简历内容节点。
- 解决方案：在预览关闭时保留仅用于打印的离屏分页内容；PDF 导出新增真实文件预览，并让生成失败后的打印入口由用户主动选择。
- 验证方式：导出对话框测试覆盖同一文件预览和下载、错误重试及关闭后的资源释放；检查专注编辑模式仍有完整分页节点，并执行正式构建。
- 相关文件：`src/App.tsx`、`src/components/PdfExportDialog.tsx`、`src/components/PdfExportDialog.test.tsx`、`src/components/StandalonePreview.tsx`、`src/export/typstPdf.ts`、`src/workspace.css`

## SR-015：窄窗口中编辑表单受挤压并产生横向溢出

- 日期：2026-09-17
- 状态：已解决
- 现象或修改背景：三栏最小宽度大于常用窗口宽度，正文编辑区过窄；内容条目的三列表单在窄编辑区被截断。
- 原因分析：工作区采用固定列宽，表单仅按窗口宽度响应，预览默认按原始 A4 宽度显示。
- 解决方案：增加专注编辑、可折叠目录及窄窗口编辑/预览切换；表单按容器宽度排列，预览支持自动适应宽度，次要文件操作收进菜单。
- 验证方式：类型检查、现有预览和设置测试；浏览器检查宽窄窗口的编辑、预览及横向溢出。
- 相关文件：`src/App.tsx`、`src/workspace.css`、`src/components/ResumePreview.tsx`、`src/settings/appSettings.ts`、`src/components/SettingsPanel.tsx`、`src/main.tsx`

## SR-001：预览关闭入口在窄窗口中不可见

- 日期：2026-09-15
- 状态：已解决
- 现象或修改背景：实时预览原本只在预览工具栏最右侧提供关闭按钮；工作台横向空间不足时，该按钮可能处于当前可视区域之外，用户无法发现或操作预览关闭功能。
- 原因分析：关闭入口依赖预览面板自身的完整可见性，没有在始终可操作的全局工具栏提供等价控制。
- 解决方案：在顶部工具栏增加带开启状态的“关闭预览 / 开启预览”切换按钮；保留预览面板原关闭按钮和关闭后的侧边恢复入口。
- 验证方式：执行完整单元测试和正式构建，并确认开关使用 `aria-pressed` 暴露当前状态。
- 相关文件：`src/App.tsx`、`src/styles.css`

## SR-002：未变化的简历可能产生重复历史快照

- 日期：2026-09-15
- 状态：已解决
- 现象或修改背景：重复点击立即备份、重新授权目录或进行全量备份时，即使简历内容没有变化，也可能在 `history` 中产生新的 JSON 文件。
- 原因分析：历史快照只记录了上次生成时间，强制备份路径没有比较简历的 `updatedAt`，并且立即备份默认总是要求创建快照。
- 解决方案：历史状态同时记录快照时间和简历版本；自动快照仅在内容变化且距上次至少十分钟时创建，立即备份只刷新 `resumes` 最新版本，首次选择新目录仍创建初始快照。
- 验证方式：补充首次快照、未变化跳过、十分钟节流和图片提取测试，并执行正式构建。
- 相关文件：`src/App.tsx`、`src/storage/diskBackup.ts`、`src/storage/diskBackup.test.ts`

## SR-003：历史快照缺少应用内查看和恢复入口

- 日期：2026-09-15
- 状态：已解决
- 现象或修改背景：磁盘备份会生成 `history` 快照，但用户只能在文件系统中看到 JSON，无法在 SwiftResume 内确认快照内容或恢复指定版本。
- 原因分析：原实现只包含快照写入和整库恢复，没有历史目录读取、版本预览及单版本恢复界面。
- 解决方案：增加历史版本管理页面，支持按简历列出快照、A4 完整预览、手动创建、删除、恢复为新简历以及覆盖原简历；覆盖前先保存当前内容的历史快照。
- 验证方式：执行完整单元测试和正式构建，检查历史读取和恢复操作的类型与界面状态。
- 相关文件：`src/App.tsx`、`src/components/HistoryPanel.tsx`、`src/components/SettingsPanel.tsx`、`src/storage/diskBackup.ts`、`src/styles.css`

## SR-004：磁盘目录未配置或权限失效时缺少启动提醒

- 日期：2026-09-15
- 状态：已解决
- 现象或修改背景：用户未选择备份目录或浏览器收回目录权限后，打开工作台只显示顶部状态文字，容易在没有磁盘保护的情况下继续编辑。
- 原因分析：启动流程虽然会恢复已保存的目录句柄并查询权限，但没有对未配置、待授权和读取失败状态提供必须处理的醒目提示。
- 解决方案：启动时复用 IndexedDB 中持久保存的目录句柄并查询读写权限；权限有效时静默继续，未配置或待授权时显示不可点背景关闭的提醒弹窗，提供选择目录、重新授权原目录和本次稍后处理。
- 验证方式：执行完整单元测试和正式构建，检查不同备份状态对应的弹窗文案和按钮路径。
- 相关文件：`src/App.tsx`、`src/components/BackupSetupPrompt.tsx`、`src/styles.css`

## SR-005：组件搭建排序低效且字段排版能力缺失

- 日期：2026-09-15
- 状态：已解决
- 现象或修改背景：组件和模块只能通过逐次点击上移、下移调整位置；标题、说明、列表以及键值字段无法分别设置字号、字重、字体和颜色。
- 原因分析：排序交互依赖离散按钮，结构化节点模型只保存文本内容，没有字段级样式数据。
- 解决方案：模块列表保留直接拖放，组件列表改用带键盘支持的拖拽排序并删除上下移动按钮；为每个语义字段增加独立文字样式，并同步到实时预览和 Typst 导出。
- 验证方式：执行类型检查、完整单元测试和正式构建；测试组件样式会生成对应 Typst 字号、字重及颜色参数。
- 相关文件：`src/components/Sidebar.tsx`、`src/components/EditorPanel.tsx`、`src/components/ResumePreview.tsx`、`src/model/resume.ts`、`src/export/typstPdf.ts`、`src/editorModes.css`

## SR-006：自由文档与组件搭建仅是同一内容的视图切换

- 日期：2026-09-15
- 状态：已解决
- 现象或修改背景：自由文档复用了组件搭建的数据和输入框，操作方式与组件搭建没有本质区别；内容块的操作栏持续占位，缺少自然输入、斜杠菜单和块级拖动体验。
- 原因分析：两种模式共享 `nodes` 模型，自由文档只是为同一组字段切换样式，没有独立的块编辑器和块数据。
- 解决方案：自由文档改用 BlockNote，提供段落、标题、列表、勾选项、引用、代码、表格、简历快捷块、斜杠菜单和悬停侧边菜单；单独保存块 JSON，并为旧自由文档自动迁移原节点和隐藏状态。
- 验证方式：执行类型检查、完整单元测试和正式构建；补充三种模式数据隔离、旧自由文档迁移、隐藏块不导出的测试。
- 相关文件：`src/components/customEditors/FreeDocumentEditor.tsx`、`src/components/customEditors/blockNoteSchema.tsx`、`src/components/customEditors/BlockDocumentPreview.tsx`、`src/model/resume.ts`、`src/export/typstPdf.ts`、`src/editorModes.css`

## SR-007：富文本编辑器能力有限且格式无法稳定进入导出

- 日期：2026-09-15
- 状态：已解决
- 现象或修改背景：原富文本依赖浏览器已废弃的 `execCommand`，仅支持少数按钮，没有字号、字体、字重、颜色、对齐、引用和表格等常用能力，部分格式还会被清洗或导出阶段丢弃。
- 原因分析：手写 `contentEditable` 没有文档模型和扩展体系，富文本白名单及 Typst 转换只覆盖了基础标签。
- 解决方案：使用 Tiptap 重构为独立的 Word 式富文本编辑器，增加完整固定工具栏；扩展安全清洗、实时预览与 Typst 转换对标题、字号、字重、颜色、高亮、对齐、引用、链接、分隔线、列表和表格的支持。
- 验证方式：执行类型检查、完整单元测试和正式构建；补充富文本样式白名单、危险标记清理、表格和安全链接保留以及 Typst 格式转换测试。
- 相关文件：`src/components/customEditors/RichTextEditor.tsx`、`src/model/richText.ts`、`src/model/richText.test.ts`、`src/export/typstPdf.ts`、`src/editorModes.css`

## SR-008：切换内容数量不同的简历时预览可能崩溃

- 日期：2026-09-15
- 状态：已解决
- 现象或修改背景：预览已经分页后切换到内容项目更少的简历，页面可能变为空白并抛出读取未定义项目属性的异常。
- 原因分析：简历切换后的第一次渲染仍短暂沿用上一份简历的分页索引，其中部分索引超出新简历的内容项目范围。
- 解决方案：渲染分页项目时忽略已失效索引，随后由布局测量正常生成新简历分页。
- 验证方式：正式构建后在浏览器切换内容数量不同的简历，页面保持可用且控制台无新增异常。
- 相关文件：`src/components/ResumePreview.tsx`

## SR-009：模块列表缺少删除入口

- 日期：2026-09-15
- 状态：已解决
- 现象或修改背景：添加简历模块后，模块列表的操作区只有显示、隐藏和复制按钮，用户无法在管理模块的位置直接删除模块。
- 原因分析：删除操作只放在单模块编辑面板顶部，模块列表组件没有接收或展示删除能力。
- 解决方案：在每个可删除模块的操作区增加删除按钮，并统一通过带模块名称的二次确认后执行删除。
- 验证方式：执行完整单元测试和正式构建，确认模块列表与编辑区域的删除入口共用同一确认逻辑。
- 相关文件：`src/App.tsx`、`src/components/Sidebar.tsx`

## SR-010：简历编辑依赖单模块页面切换

- 日期：2026-09-15
- 状态：已解决
- 现象或修改背景：编辑区一次只显示一个模块的表单，修改不同模块时需要在侧栏反复切换，无法结合整份简历的上下文确认内容和顺序。
- 原因分析：编辑器和实时预览是两个完全分离的渲染路径，模块选中状态直接决定唯一显示的表单，没有连续文档画布和展示、编辑状态的切换机制。
- 解决方案：将中间区域重构为不分页的连续简历画布；侧栏点击仅滚动定位且不关闭编辑，点击画布中的个人信息或模块后原位切换为编辑表单，同时只允许一个区域编辑；切换模块、点击完成或点击简历纸张外围的灰色背景时提交当前内容，纸张内部留白和其他界面区域不改变编辑状态。
- 验证方式：执行完整单元测试和正式构建，并在浏览器中验证连续展示、侧栏定位、单模块原位编辑、模块切换保存及灰色外围点击退出行为。
- 相关文件：`src/App.tsx`、`src/components/EditorPanel.tsx`、`src/components/ResumeEditorCanvas.tsx`、`src/components/ResumePreview.tsx`、`src/styles.css`

## SR-011：新建简历缺少默认内容结构

- 日期：2026-09-15
- 状态：已解决
- 现象或修改背景：新建简历后只有固定的个人信息区域，用户需要逐个添加常见模块才能开始填写一份完整简历。
- 原因分析：新建流程直接使用完全空白文档，未提供面向首次填写的起始模板。
- 解决方案：新增不含虚构示例内容的起始简历模板，默认创建求职意向、工作经历、项目经历、教育背景、专业技能和自我评价模块；完全空白文档能力仍保留在数据模型中。
- 验证方式：补充模型测试，验证默认模块的类型、顺序、标题和无示例数据特征，并执行完整单元测试和正式构建。
- 相关文件：`src/App.tsx`、`src/model/resume.ts`、`src/model/resume.test.ts`

## SR-012：隐藏模块仍出现在整页编辑画布

- 日期：2026-09-15
- 状态：已解决
- 现象或修改背景：从侧栏隐藏模块后，右侧分页预览会隐藏该模块，但中间的整页简历画布仍以半透明样式展示它，导致两个简历展示结果不一致。
- 原因分析：整页画布遍历了全部模块，仅用样式标记隐藏状态，没有按 `enabled` 状态过滤。
- 解决方案：整页画布只渲染已启用模块；隐藏操作仍保留在侧栏，若正在编辑的模块被隐藏，则保存内容并结束该模块的编辑状态。
- 验证方式：补充整页画布隐藏模块回归测试，并执行完整单元测试和正式构建。
- 相关文件：`src/components/ResumeEditorCanvas.tsx`、`src/components/ResumeEditorCanvas.test.tsx`、`src/styles.css`

## SR-013：侧栏切换模块不会切换就地编辑器

- 日期：2026-09-16
- 状态：已解决
- 现象或修改背景：正在画布中编辑一个模块时，点击侧栏中的另一个模块只会改变定位状态，原模块仍保持编辑，用户还需要再次点击画布才能切换。
- 原因分析：侧栏选择状态由应用层管理，就地编辑状态则封装在画布组件内部，两者无法协调保存、关闭和打开顺序。
- 解决方案：将就地编辑状态提升到应用层统一管理；侧栏选择可见模块时先保存当前编辑内容，再关闭旧模块、打开目标模块并滚动定位，同时保证任意时刻只有一个编辑器打开。
- 验证方式：补充受控编辑状态回归测试，并执行完整单元测试和正式构建。
- 相关文件：`src/App.tsx`、`src/components/ResumeEditorCanvas.tsx`、`src/components/ResumeEditorCanvas.test.tsx`

## SR-013：经历首行与自由正文被割裂为互斥编辑模式

- 日期：2026-09-16
- 状态：已解决
- 现象或修改背景：项目模块能稳定显示名称、职责和日期，但正文标签及结构写死；组件搭建无法在同一段内自由加粗；自由文档和富文本又无法稳定表达右对齐日期。用户必须在三种各有缺口的模式中选择，仍不能自然完成常见简历条目。
- 原因分析：数据模型按编辑器技术形态拆分，而不是按简历内容语义建模；标题日期行和自由正文分别归属不同模型，并在编辑、预览及 PDF 中维护了多条渲染路径。
- 解决方案：升级为不兼容的 schema v2，删除三模式及 BlockNote 实现；项目、工作和自定义模块统一为可重复的内容条目，每条由可选标题、补充信息、日期和 Tiptap JSON 正文组成。首行全部留空时只渲染正文，预览与 Typst 直接消费同一数据语义。
- 验证方式：补充模型、预览和 Typst 测试，执行完整单元测试与正式构建，并在浏览器验证标题日期对齐、纯正文模块、局部加粗及列表编辑。
- 相关文件：`src/model/resume.ts`、`src/model/contentRichText.ts`、`src/components/EditorPanel.tsx`、`src/components/Sidebar.tsx`、`src/components/ResumePreview.tsx`、`src/components/customEditors/ContentBodyEditor.tsx`、`src/export/typstPdf.ts`、`src/editorModes.css`

## SR-014：统一内容模块后富文本编辑能力严重退化

- 日期：2026-09-16
- 状态：已解决
- 现象或修改背景：统一内容模块最初只保留了加粗、斜体、下划线、列表和链接等基础按钮，无法设置字号、字体、字重、颜色、高亮、行高、段落缩进、对齐和表格，较替换前的成熟富文本编辑器明显退化。
- 原因分析：实现时错误地把“统一内容数据结构”理解成“限制正文编辑能力”，在迁移到 JSON 文档后没有恢复已有的 Tiptap 官方扩展和完整工具栏。
- 解决方案：保留可选标题日期行与正文的统一结构，恢复 Tiptap 的文字样式、对齐、高亮和表格扩展；列表缩进使用编辑器原生命令，普通段落仅增加可持久化的缩进属性，并同步扩展安全预览和 Typst 导出。
- 验证方式：补充高级格式 JSON 渲染、安全样式清洗及 Typst 格式转换测试，执行完整单元测试和正式构建。
- 相关文件：`src/components/customEditors/ContentBodyEditor.tsx`、`src/model/contentRichText.ts`、`src/model/richText.ts`、`src/model/contentRichText.test.ts`、`src/model/richText.test.ts`、`src/export/typstPdf.ts`、`src/export/typstPdf.test.ts`、`src/editorModes.css`
